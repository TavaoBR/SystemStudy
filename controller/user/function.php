...
function verifica_sessao(){
     if(!$_SESSION['usuarioID'] AND !$_SESSION['usuarioUser'] AND !$_SESSION['usuarioSenha']){
          $_SESSION['area_restrista'] = "<div class='alert alert-danger'> Logue para acessar a pagina</div>";
...
function perfil_exibir(){

    include_once("../../model/conection/conexao.php");

    $user_id = filter_input(INPUT_GET, 'id_user', FILTER_SANITIZE_NUMBER_INT);
    $select_user_ = "SELECT * FROM user AS usuario   
    INNER JOIN user_informacoes AS usuario_info ON usuario_info.fk_user = usuario.id
    INNER JOIN rede_social AS redesocial ON redesocial.fk_user = usuario.id
    WHERE usuario.id = :id
    ";
